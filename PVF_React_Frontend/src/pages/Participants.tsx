...
const stationTemplates = [
  {
    ...
    description:
      "Verify registration, confirm consent, and assign the participant ID.",
  },
  {
    id: "vision-screening",
    title: "Station 2 – Vision Screening",
    description:
      "Record vision screening results and route the participant to the next step.",
  ...
  },
  {
    ...
    description:
      "Provide a full exam and note prescription or referral outcomes.",
  ...
  },
  {
    ...
    description:
      "Record prescription details and frame selections for ordering.",
  },
  {
    id: "vision-success",
    title: "Station 5 – Vision Success",
    description:
      "Confirm next steps, referrals, and follow-up resources before departure.",
    // if no on station 2, jump to station 5
  },
];

const stationOrder = new Map<string, number>(
  stationTemplates.map((station, index) => [station.id, index]),
);
...
function buildNewEvent(
  ...
  return {
    ...
    registrationEventId: registrationEventId || undefined,
  };
}
...
export default function Participants() {
  ...
  const [participantProfile, setParticipantProfile] =
    useState<ParticipantProfile>(createDefaultParticipantProfile());
  ...
  >([]);
  const [registrationEventsLoading, setRegistrationEventsLoading] =
    useState(true);
  const [selectedRegistrationEventId, setSelectedRegistrationEventId] =
    useState("");
  const [participantEventFilterId, setParticipantEventFilterId] =
    useState("all");
    ...
    async function loadData() {
      ...
      if (customerKeyParam) {
        ...
        if (match) {
          ...
          if (
            eventIdParam &&
            match.Events?.some((event) => event.id === eventIdParam)
          ) {
  ...
  const participantEvents = useMemo(
    () => selectedCustomer?.Events ?? [],
    [selectedCustomer],
  );
  const sortedParticipantEvents = useMemo(
    () => [...participantEvents].sort(sortEventsByRecency),
    [participantEvents],
  );
  ...
  const selectedRegistrationEvent = useMemo(
    () =>
      registrationEvents.find(
        (event) => event.id === selectedRegistrationEventId,
      ) ?? null,
    ...
    registrationEvents.forEach((event) => {
      ...
      options.push({
        ...
        label: event.eventDate
          ? `${event.eventName || "Untitled event"} (${event.eventDate})`
  ...
  }, [customers, searchTerm, participantEventFilterId, registrationEvents]);
  ...
  useEffect(() => {
    ...
    const filterStillExists = participantEventFilterOptions.some(
      (option) => option.id === participantEventFilterId,
    );
  ...
  useEffect(() => {
    ...
    const eventIdParam = searchParams.get("eventId")?.trim() ?? "";
    if (eventIdParam) {
      const matchingEvent = participantEvents.find(
        (event) => event.id === eventIdParam,
      );
      if (matchingEvent) {
  ...
  const handleDeleteParticipant = async () => {
    ...
    const confirmed = window.confirm(
      `Delete participant ${selectedCustomer.Email || selectedCustomer.id}? This removes their record and all saved events.`,
    );
    if (!confirmed) {
      return;
    }
  ...
  const normalizeProfilePath = (path: string) => {
    return path.startsWith("participant.")
      ? path.slice("participant.".length)
      : path;
  };
  ...
  const handleProfileChange = (
    ...
    setParticipantProfile((current) => {
      ...
      segments.slice(0, -1).forEach((segment) => {
        const currentTarget = target[segment] as
  ...
  const handleProfileArrayToggle = (path: string, value: string) => {
    setParticipantProfile((current) => {
      ...
      const lastSegment = segments[segments.length - 1];
      const currentValues = Array.isArray(target[lastSegment])
        ? (target[lastSegment] as string[])
        : [];
  ...
  const handleAddEventToAll = async () => {
    ...
    const selectedParticipantId =
      ...
      selectedCustomer?.Email ??
      "";
    const selectedCustomerEvent = buildNewEvent(
    ...
    const nextCustomers = customers.map((customer) => {
      ...
      const participantId =
        customer.participant?.id ?? customer.id ?? customer.Email;
      const newEvent =
        customer.Email.toLowerCase() === selectedCustomer?.Email?.toLowerCase()
  ...
  const updateEyeExamDecision = async (
    eventId: string,
  ...
  const handleAdvanceEvent = async (eventId: string, eventStatus: string) => {
    ...
    try {
      ...
      const currentEvent = participantEvents.find(
      ...
      const orderedStationStatuses = sortStationStatuses(
        currentEvent.stationStatuses,
      );
      const currentStation = orderedStationStatuses.find(
      ...
      if (currentStation.id === "vision-success") {
        const stationStatuses: StationStatus[] = sortStationStatuses(
          orderedStationStatuses.map((station) =>
            station.id === "vision-success"
      ...
      if (
        currentStation.id === "vision-screening" &&
        (!currentStation.decision ||
          (currentStation.decision !== "PASS" &&
            currentStation.decision !== "FAIL"))
      ) {
        return;
      }

      if (
        currentStation.id === "eye-exam" &&
        (!currentStation.decision ||
          (currentStation.decision !== "REFERRAL" &&
            currentStation.decision !== "FRAME"))
      ) {
        return;
      }

      if (currentStation.id === "frame-selection") {
        const stationThree = orderedStationStatuses.find(
          (station) => station.id === "eye-exam",
        );
        const stationFour = orderedStationStatuses.find(
          (station) => station.id === "frame-selection",
        );
        ...
        if (
          stationThree.decision === "REFERRAL" &&
          !stationFour.pboReferralConfirmed
        ) {
          return;
        }

        if (
          stationThree.decision === "FRAME" &&
          !stationFour.frameSelection?.trim()
        ) {
          return;
        }
      }

      const currentIndex = orderedStationStatuses.findIndex(
        (station) => station.status === "current",
      );
      const stationStatuses: StationStatus[] = sortStationStatuses(
        orderedStationStatuses.map((station, index): StationStatus => {
          ...
          if (
            currentStation.id === "vision-screening" &&
            currentStation.decision === "PASS" &&
            (station.id === "eye-exam" || station.id === "frame-selection")
          ) {
      ...
      if (
        ...
        nextIndex = stationStatuses.findIndex(
          (station) => station.id === "vision-success",
        );
      }
      ...
      if (nextIndex >= 0) {
        stationStatuses[nextIndex] = {
          ...stationStatuses[nextIndex],
          status: "current",
        };
      }

      const isCompleted = stationStatuses.every(
        (station) =>
          station.status === "complete" || station.status === "skipped",
      );
  ...
  const getProgress = (event: EventRecord) => {
    const completed = event.stationStatuses.filter(
      (station) =>
        station.status === "complete" || station.status === "skipped",
    ).length;
  ...
  const handlePrintStationSummary = (event: EventRecord) => {
    const stationTwo = event.stationStatuses.find(
      (station) => station.id === "vision-screening",
    );
    ...
    const isYes = (value: string | undefined) =>
      (value ?? "").toLowerCase() === "yes";
    const isNo = (value: string | undefined) =>
      (value ?? "").toLowerCase() === "no";

    const frameSelection = event.stationStatuses.find(
      (station) => station.id === "frame-selection",
    );
  ...
  const sections = [
    ...
    },
    {
      ...
      fields: [
        ...
        {
          key: "participant.demographics.primaryLanguage",
          label: "Primary Language",
        },
        {
          key: "participant.demographics.veteranStatus",
          label: "Veteran Status",
        },
        {
          key: "participant.demographics.lgbtqIdentity",
          label: "LGBTQ+ Identity",
        },
        {
          key: "participant.demographics.disabilityStatus",
    ...
    },
    {
      ...
      fields: [
        {
          key: "participant.contact.preferredCommunication",
    ...
    },
    {
      ...
      fields: [
        {
          key: "participant.visionIntake.wearsGlasses",
          label: "Wears glasses",
          type: "select" as const,
          options: yesNoUnsure,
        },
        {
          key: "participant.visionIntake.glassesStatus",
          label: "Glasses status",
        },
        {
          key: "participant.visionIntake.wearsContacts",
          label: "Wears contacts",
          type: "select" as const,
          options: yesNoUnsure,
        },
        {
          key: "participant.visionIntake.lastEyeExam",
          label: "Last eye exam",
          type: "date" as const,
        },
        {
          key: "participant.visionIntake.eyeCareProvider",
          label: "Eye care provider",
        },
        {
          key: "participant.visionIntake.toldNeedsGlasses",
          label: "Told they need glasses",
          type: "select" as const,
          options: yesNoUnsure,
        },
        {
          key: "participant.insurance.visionInsurance",
          label: "Vision insurance",
          type: "select" as const,
          options: yesNoUnsure,
        },
        {
          key: "participant.insurance.medicalInsuranceProvider",
          label: "Medical insurance provider",
        },
        {
          key: "participant.referralSource",
        ...
        {
          key: "participant.consents.signatureDate",
          label: "Signature date",
          type: "date" as const,
        },
        {
          key: "participant.checkedIn",
          label: "Checked in",
      ...
      return (
        ...
        >
          <input
            ...
            onChange={(event) =>
              handleProfileChange(field.key, event.target.checked)
            }
          />
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-2">
            Browse all participants, update their profile details, and track
            station-based event progress.
          </p>
        </div>
        <Link to="/participants" className="text-blue-700 underline">
          View all
        </Link>
      </div>
      ...
      <div className="grid lg:grid-cols-[280px_1fr] gap-6">
        <aside className="border rounded-lg p-4 bg-white shadow-sm">
          ...
          <select
            ...
            onChange={(event) =>
              setParticipantEventFilterId(event.target.value)
            }
            className="mb-3 w-full rounded border px-3 py-2 text-sm"
          >
              ...
              {editing ? (
                  ...
                  {sections.map((section) => (
                    <Accordion
                      ...
                      onChange={(_, isExpanded) =>
                        setExpanded(isExpanded ? section.id : "section-1")
                      }
                    >
                      <AccordionSummary
                  ...
                  <div className="mt-6 rounded-lg border border-gray-200 bg-gray-50 p-4">
                    ...
                    <div className="mt-4 grid gap-4 md:grid-cols-2">
                      <div className="md:col-span-2 rounded border border-gray-200 bg-white p-3">
                        ...
                        <div className="mt-3 grid gap-2 md:grid-cols-2">
                          {visionConcernOptions.map((concern) => (
                            ...
                            >
                              <input
                                type="checkbox"
                                checked={participantProfile.visionIntake.currentConcerns.includes(
                                  concern,
                                )}
                                onChange={() =>
                                  handleProfileArrayToggle(
                                    "participant.visionIntake.currentConcerns",
                                    concern,
                                  )
                                }
                                className="mt-1"
                        ...
                        <input
                          value={
                            participantProfile.visionIntake.currentConcernsOther
                          }
                          onChange={(event) =>
                            handleProfileChange(
                              "participant.visionIntake.currentConcernsOther",
                              event.target.value,
                            )
                          }
                        ...
                        <div className="mt-3 grid gap-2 md:grid-cols-2">
                          {resourceOptions.map((resource) => (
                            ...
                            >
                              <input
                                type="checkbox"
                                checked={participantProfile.resourceInterests.includes(
                                  resource,
                                )}
                                onChange={() =>
                                  handleProfileArrayToggle(
                                    "participant.resourceInterests",
                                    resource,
                                  )
                                }
                                className="mt-1"
                      ...
                      <div className="md:col-span-2 rounded border border-gray-200 bg-white p-3">
                        ...
                        <div className="mt-3 space-y-2">
                          {[
                            [
                              "participant.consents.consentToParticipate",
                              "Consent to participate",
                            ],
                            [
                              "participant.consents.photoVideoRelease",
                              "Photo/video release",
                            ],
                            [
                              "participant.consents.communicationAuthorization",
                              "Communication authorization",
                            ],
                            [
                              "participant.consents.acknowledgement",
                              "Acknowledgement",
                            ],
                          ].map(([path, label]) => (
                            <label
                ...
                <div className="space-y-6">
                  <div className="grid md:grid-cols-2 gap-4 text-sm text-gray-700">
                    ...
                    <div>
                      <span className="font-semibold">Age at event:</span>{" "}
                    ...
                    <div>
                      ...
                      {participant.address.streetAddress
                        ? `${participant.address.streetAddress}, ${participant.address.city}, ${participant.address.state} ${participant.address.zipCode}`.trim()
                        : "—"}
                    </div>
                    <div>
                    ...
                    <div>
                      <span className="font-semibold">Vision insurance:</span>{" "}
                      {participant.insurance.visionInsurance || "—"}
                    </div>
                    <div>
                      <span className="font-semibold">Resource interests:</span>{" "}
                      {participant.resourceInterests.length
                        ? participant.resourceInterests.join(", ")
                        : "—"}
                    </div>
                  </div>
                      ...
                      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            Choose a shared event from registration and add it
                            to one participant or everyone.
                          </p>
                        </div>
                        <div className="grid gap-3 sm:min-w-[360px]">
                          <label className="block text-sm font-medium text-gray-700">
                            ...
                            <select
                              ...
                              onChange={(event) =>
                                setSelectedRegistrationEventId(
                                  event.target.value,
                                )
                              }
                            ...
                            >
                              {registrationEventsLoading ? (
                                ...
                                <option value="">
                                  No registration events available
                                </option>
                              ) : (
                  ...
                  {sortedParticipantEvents.length === 0 ? (
                    <div className="rounded border border-dashed border-gray-300 p-4 text-sm text-gray-600">
                      No events yet. Add an event to begin tracking the
                      participant’s station progress.
                    </div>
                  ) : (
                    <div className="space-y-4">
                      {sortedParticipantEvents.map((event) => {
                        const orderedStatuses = sortStationStatuses(
                          event.stationStatuses,
                        );
                        const currentStation = orderedStatuses.find(
                          (station) => station.status === "current",
                        );
                        ...
                        return (
                          <Accordion
                            ...
                            onChange={(_, isExpanded) =>
                              setExpandedEventId(isExpanded ? event.id : null)
                            }
                            ...
                            <AccordionDetails>
                              <div className="rounded-lg border border-gray-200 p-4">
                                            ...
                                            <select
                                              ...
                                              onChange={(changeEvent) =>
                                                updateEventStation(
                                                  ...
                                                  station.id,
                                                  {
                                                    decision: changeEvent.target
                                                      .value as StationDecision,
                                                  },
                                                )
                                              }
                                              className="mt-1 w-full rounded border px-3 py-2"
                                      ...
                                      {station.id === "frame-selection" && (
                                        <div className="mt-3 rounded border border-gray-200 bg-white p-3">
                                          {orderedStatuses.find(
                                            ...
                                            <p className="text-sm text-gray-600">
                                              Select an exam outcome on Station
                                              3 first.
                                            </p>
                                          )}
                                        </div>
                                ...
                                <div className="mt-4 flex flex-wrap gap-2">
                                  ...
                                  <button
                                    onClick={() =>
                                      void handleResetEvent(event.id)
                                    }
