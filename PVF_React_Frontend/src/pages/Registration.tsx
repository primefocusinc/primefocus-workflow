...
function Section({
  ...
  return (
    <Accordion
      defaultExpanded
      disableGutters
      className="rounded-lg border border-gray-200 bg-white shadow-sm before:hidden"
    >
...
export default function Registration() {
  ...
  useEffect(() => {
    ...
    const requestedEvent = requestedEventId
      ? availableRegistrationEvents.find(
          (event) => event.id === requestedEventId,
        )
      : undefined;
    const selectedStillExists = availableRegistrationEvents.some(
      (event) => event.id === form.event,
    );
    ...
    if (
      !form.event ||
      !selectedStillExists ||
      (requestedEvent && form.event !== requestedEvent.id)
    ) {
  ...
  const phoneInvalid =
    form.guardianPhone.trim() !== "" && !isValidPhoneNumber(form.guardianPhone);
  const emailInvalid =
    form.guardianEmail.trim() !== "" && !isValidEmail(form.guardianEmail);
  ...
  const toggleArrayValue = (
    field: "currentConcerns" | "resourceInterests",
    value: string,
  ) => {
    setForm((current) => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const selectedEvent = availableRegistrationEvents.find(
      (event) => event.id === form.event,
    );
    const payload = createRegistrationSubmissionPayload(
      form,
      selectedEvent?.eventName || defaultEventName,
      selectedEvent?.id,
    );
    ...
    try {
      ...
      setSaveStatus("error");
      setSaveError(
        "Unable to save this registration to the database. Please try again or contact an administrator.",
      );
    }
  };
  ...
  const inputClass =
    "mt-1 w-full rounded border border-gray-300 px-3 py-2 text-gray-900 shadow-sm focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-100";
  const selectClass = `${inputClass} bg-white`;
  const invalidInputClass = `${inputClass} border-red-500 bg-red-50 focus:border-red-600 focus:ring-red-100`;
  const canReviewRegistration = Boolean(
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_280px]">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-gray-700">
              Children ages 5-17 receive priority scheduling. Adults are welcome
              to register and may be scheduled if appointment availability
              permits.
            </p>
          </div>
          ...
          <aside className="rounded-lg border border-blue-100 bg-white p-5 shadow-sm">
            ...
            <p className="mt-2 text-sm text-gray-600">
              Complete required fields to save the registration.
            </p>
          </aside>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-6">
              ...
              >
                {availableRegistrationEvents.map((eventOption) => (
                  <option key={eventOption.id} value={eventOption.id}>
                    ...
                    {eventOption.eventDate ? `(${eventOption.eventDate})` : ""}
                  </option>
                ))}
              </select>
            </label>
          ...
          <Section eyebrow="Section 2" title="Participant Information">
            {[
              ...
              <label key={String(field)}>
                <FieldLabel required={Boolean(required)}>
                  {String(label)}
                </FieldLabel>
                <input
                  type={
          ...
          <Section eyebrow="Section 5" title="Vision History">
            <label>
              ...
              <select
                value={form.wearsGlasses}
                onChange={handleInput("wearsGlasses")}
                className={selectClass}
                required
              >
            ...
            {form.wearsGlasses === "Yes" ? (
              <>
                <label>
                  ...
                  <select
                    value={form.glassesStatus}
                    onChange={handleInput("glassesStatus")}
                    className={selectClass}
                  >
                ...
                {form.glassesStatus === "Other" ? (
                  <label>
                    ...
                    <input
                      value={form.glassesStatusOther}
                      onChange={handleInput("glassesStatusOther")}
                      className={inputClass}
                    />
                  </label>
                ) : null}
            ...
            <label>
              <FieldLabel required>
                Does the participant wear contact lenses?
              </FieldLabel>
              <select
                value={form.wearsContacts}
                onChange={handleInput("wearsContacts")}
                className={selectClass}
                required
              >
                <option value="">Select one</option>
            ...
            <label>
              ...
              <input
                ...
                onChange={handleInput("lastEyeExam")}
                className={inputClass}
              />
            </label>
            ...
            <label>
              ...
              <input
                value={form.eyeCareProvider}
                onChange={handleInput("eyeCareProvider")}
                className={inputClass}
              />
            </label>
            ...
            <label>
              <FieldLabel required>
                Has the participant ever been told they need glasses?
              </FieldLabel>
              <select
                ...
                {currentConcernOptions.map((concern) => (
                  ...
                  >
                    <input
                      ...
                      onChange={() =>
                        toggleArrayValue("currentConcerns", concern)
                      }
                      className="mt-1"
                    />
            ...
            <p className="rounded bg-blue-50 p-3 text-sm text-blue-900 md:col-span-2">
              The condition or availability of glasses will not prevent
          ...
          <Section eyebrow="Section 6" title="Insurance Information">
            <div className="rounded bg-green-50 p-3 text-sm text-green-900 md:col-span-2">
              Insurance information is optional, collected for program planning
              and reporting only. Prime Focus Inc. will not bill insurance, and
              insurance status will not affect eligibility.
            </div>
            <label>
              <FieldLabel required>
          ...
          <Section eyebrow="Section 7" title="Community Resource Interests">
            <fieldset className="md:col-span-2">
              <legend className="text-sm font-semibold text-gray-800">
                Would you like information about any of the following community
                resources?
              </legend>
                ...
                {resourceOptions.map((resource) => (
                  ...
                  >
                    <input
                      ...
                      onChange={() =>
                        toggleArrayValue("resourceInterests", resource)
                      }
                      className="mt-1"
                    />
          ...
          <Section eyebrow="Section 9" title="Consents & Authorizations">
            <div className="space-y-3 md:col-span-2">
              {[
                ...
                <label
                  key={String(field)}
                  className="flex gap-3 rounded border border-gray-200 bg-gray-50 p-3"
                >
                  <input
                    type="checkbox"
            ...
            <label>
              ...
              <input
                ...
                onChange={handleInput("printedName")}
                className={inputClass}
                required
              />
            </label>
            <label>
              ...
              <input
                ...
                onChange={handleInput("signatureDate")}
                className={inputClass}
                required
              />
            </label>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
            <div>
              ...
              <p className="text-sm text-gray-600">
                This page is ready to save the registration to the participant
                database.
              </p>
            </div>
            <div className="flex gap-3">
              ...
              <Button
                type="submit"
                variant="contained"
                disabled={!canReviewRegistration || saveStatus === "saving"}
              >
        ...
        >
          ...
          <DialogContent>
            <p className="text-sm text-gray-700">
              A confirmation was recorded for{" "}
              {createdUserEmail || form.guardianEmail.trim().toLowerCase()}.
            </p>
          </DialogContent>
          <DialogActions>
            <Button
