...
export const createRegistrationSubmissionPayload = (
  form: RegistrationForm,
  requestedEventName?: string,
  requestedEventId?: string,
  ...
  return {
    participant: {
      ...
      guardian: {
        name:
          form.participantType === childParticipantType
            ? normalizeText(form.guardianName)
            : "",
        relationship:
          form.participantType === childParticipantType
            ? normalizeText(form.guardianRelationship)
            : "",
        phoneNumber:
          form.participantType === childParticipantType ? normalizedPhone : "",
        email:
          form.participantType === childParticipantType ? normalizedEmail : "",
      },
      contact: {
      ...
      school: {
        name:
          form.participantType === childParticipantType
            ? normalizeText(form.schoolName)
            : "",
        district:
          form.participantType === childParticipantType
            ? normalizeText(form.schoolDistrict)
            : "",
        currentGrade:
          form.participantType === childParticipantType
            ? normalizeText(form.currentGrade)
            : "",
      },
      visionIntake: {
...
export function createCustomerRecordFromPayload(
  payload: RegistrationSubmissionPayload,
): CustomerRecord {
  ...
  return {
    ...
    "Preferred Method of Communication":
      participant.contact.preferredCommunication,
